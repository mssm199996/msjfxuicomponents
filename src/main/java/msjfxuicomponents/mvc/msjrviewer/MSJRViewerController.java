...
public class MSJRViewerController implements Initializable {
	...
	private WritableImage getImage(int pageNumber) throws JRException {
		BufferedImage bufferedImage = (BufferedImage) JasperPrintManager.printPageToImage(this.jasperPrint, pageNumber,
				2);

		return this.toFXImage(bufferedImage);
	}

	// ---------------------------------------------------------------

	private WritableImage toFXImage(BufferedImage bimg) {
		...
		IntegerComponentRaster icr = (IntegerComponentRaster) converted.getRaster();
