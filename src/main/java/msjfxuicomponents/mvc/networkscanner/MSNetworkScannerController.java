...
public class MSNetworkScannerController implements Initializable {
	...
	@FXML
	private JFXTextField centralServerIp;
	...
	void confirmHandler() {
		...
		if (remoteMachine != null || !this.centralServerIp.getText().equals("")) {
			if (remoteMachine == null)
				remoteMachine = new RemoteMachine();

			remoteMachine.setIpAddress(this.centralServerIp.getText());
			...
			alert.setContentText(
					"Veuillez d'abords choisir une machine ou bien saisir l'adresse IP du serveur central !");
	...
	private void initTextFields() {
		Platform.runLater(() -> {
			...
			this.centralServerIp.setText(this.getWindow().getInitialCentralIpServer());
			this.port.setText(this.getWindow().getInitialServerPort());
		});
	}

	private void initTables() {
		this.remoteMachinesTable.getSelectionModel().selectedItemProperty()
				.addListener((observable, oldValue, newValue) -> {
					this.centralServerIp.setText("");

					if (newValue != null)
						this.centralServerIp.setText(newValue.getIpAddress());
				});
		...
		this.remoteMachinesTable.setRowFactory(call -> {
			return new DoubleClickingRow<RemoteMachine>() {
				...
				public void onMouseDoubleClicked(RemoteMachine remoteMachine) {
					remoteMachine.setIpAddress(centralServerIp.getText());
