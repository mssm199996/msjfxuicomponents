...
public abstract class AutoCommitTextFieldCell<T, S> extends TextFieldTableCell<T, S> {
	...
	public boolean confirmEditing(T entity) {
		return true;
	}
	...
	public void startEdit() {
		...
		T entity = this.getTableView().getItems().get(this.getTableRow().getIndex());

		if (!this.confirmEditing(entity))
			return;
	...
	public void commitEdit(S newValue) {
		...
		if (this.enableCommit(entity, this.oldValue, newValue)) {
			...
			super.commitEdit(observableValue.getValue());
	}

	public abstract void onEditCommit(S newValue);

	public boolean enableCommit(T entity, S oldValue, S newValue) {
		return true;
	}
